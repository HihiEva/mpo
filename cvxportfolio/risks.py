...
class ShrinkageSigma(BaseRiskModel):
    """
    Ledoit-Wolf 收缩估计风险模型。
    
    该模型在每个时间点 t，使用过去 lookback 长度的历史收益率，
    自动计算最优收缩系数 delta，生成稳健的协方差矩阵 Sigma。
    
    Risk = w.T * Sigma_shrunk * w
    """

    def __init__(self, returns, lookback=252, **kwargs):
        """
        Args:
            returns (pd.DataFrame): 资产历史收益率数据 (index为时间，columns为资产)
            lookback (int): 回看窗口长度 (例如 60, 252)
        """
        self.returns = returns
        self.lookback = lookback
        # 确保传入的数据没有 NaN，或者在外部已经清洗过
        # 如果这里报错，说明外部清洗没覆盖到
        assert not pd.isnull(returns).values.any(), "Returns data contains NaN. Please clean data before initializing."
        super(ShrinkageSigma, self).__init__(**kwargs)

    def _estimate(self, t, wplus, z, value):
        t_idx = self.returns.index.searchsorted(t)
        start_idx = max(0, t_idx - self.lookback)
        window_returns = self.returns.iloc[start_idx:t_idx]
        
        # 1. 修复数据不足时的逻辑，防止 Unbounded
        if len(window_returns) < 20:
            # 数据不足时，给定一个极大惩罚或零矩阵表达式，不能只返回 0.0
            # 返回 0.0 会破坏 cvxpy 结构，这里返回全 0 的 Parameter，并在外部通过约束防止无界
            dummy_sigma = np.zeros((wplus.shape[0]-1, wplus.shape[0]-1))
            return cvx.quad_form(wplus[:-1], cvx.Parameter(shape=dummy_sigma.shape, value=dummy_sigma, PSD=True))
        ...
        try:
            ...
            logger.warning(f"LedoitWolf failed at time {t}, falling back to empirical cov: {e}")
            Sigma_shrunk = np.cov(X, rowvar=False)

        # 👉 2. 修复维度错位：必须切片 wplus[:-1]，将最后一位现金列剔除
        self.expression = cvx.quad_form(wplus[:-1], cvx.Parameter(shape=Sigma_shrunk.shape, value=Sigma_shrunk, PSD=True))
